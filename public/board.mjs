import {COLORS,COLOR_NAMES,key,neighbors,legalTargets,provinceAt,defense} from '/shared/engine.mjs';
...
export class Board {
  constructor(element,onCell){
    ...
    this.svg.addEventListener('click',e=>{if(this.dragged)return;const id=e.target.closest('[data-cell]')?.dataset.cell;if(id)this.onCell(id);});
  ...
  reset(){this.zoom=matchMedia('(max-width:700px)').matches?1.3:1;this.pan={x:0,y:0};this.transform();}
  transform(){this.world.setAttribute('transform',`scale(${this.zoom}) translate(${this.pan.x} ${this.pan.y})`);}
  draw(map,{editing=false,selected=null,game=null,selection=null,build=null,showDefense=false}={}){
    ...
    const targets=new Set(game&&selection?legalTargets(game,selection,build?.level):[]);
    const province=game&&selected?provinceAt(game,selected):null;
    let html='';
    for(const[id,c]of tiles){
      const {x,y}=center(c),fill=c.owner>=0?COLORS[c.owner]:'#c4ce99';
      ...
      if(province?.cells.includes(id))html+=`<polygon points="${points(x,y,23)}" fill="none" stroke="#fff8dd" stroke-width=".7" opacity=".6"/>`;
      if(targets.has(id))html+=`<circle cx="${x}" cy="${y+10}" r="3" fill="#fffde7" stroke="#476d48" stroke-width=".8"/>`;
      ...
      if(selected===id)html+=`<polygon class="selection-ring" points="${points(x,y,23)}"/>`;
      html+='</g>';
    }
    ...
    for(const c of all){const {x,y}=center(c);for(let i=0;i<6;i++){
      ...
      if(!n||n.owner!==c.owner){const[a1,a2]=pair.map(v=>v*Math.PI/3);html+=`<path d="M${x+SIZE*Math.cos(a1)} ${y+SIZE*Math.sin(a1)}L${x+SIZE*Math.cos(a2)} ${y+SIZE*Math.sin(a2)}" stroke="${!n?'#274e49':'#486344'}" stroke-width="${!n?1.8:1.3}" opacity="${!n?.85:.55}" pointer-events="none"/>`;}
    }}
